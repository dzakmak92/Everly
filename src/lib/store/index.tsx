...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (active && raw) setEntries(JSON.parse(raw));
      } catch {
  ...
  const addEntry = useCallback((kind: EntryKind, note?: string) => {
    setEntries((prev) => [{ id: newId(), kind, at: new Date().toISOString(), note: note?.trim() || undefined }, ...prev]);
  }, []);
  ...
  const value = useMemo<DataValue>(
    () => ({ loading, entries, addEntry, deleteEntry, clearAll }),
    [loading, entries, addEntry, deleteEntry, clearAll],
  );
