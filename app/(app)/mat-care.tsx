    ...
    <ScrollView style={{ flex: 1, backgroundColor: color.canvas }} contentContainerStyle={{ paddingTop: insets.top + 8, paddingBottom: insets.bottom + 28, paddingHorizontal: 22, gap: 16 }}>
      ...
      <View style={[{ backgroundColor: '#fff', borderRadius: radius.card, padding: 16, gap: 10 }, shadow.card]}>
        ...
        <View style={{ flexDirection: 'row', gap: 6 }}>
          {COMFORT.map((c, i) => (
            <Pressable key={c} onPress={() => { addMomCare({ kind: 'comfort', value: i }); toast('Comfort logged'); }} style={{ flex: 1, paddingVertical: 10, borderRadius: radius.tile, alignItems: 'center', backgroundColor: lastComfort?.value === i && isToday(lastComfort.at) ? color.maternalTeal : color.canvas }}>
        ...
        <Pressable onPress={() => setSleepOpen(false)} style={{ flex: 1, backgroundColor: 'rgba(40,18,50,0.35)', justifyContent: 'center', paddingHorizontal: 28 }}>
          <Pressable onPress={() => {}} style={[{ backgroundColor: color.canvas, borderRadius: radius.card, padding: 20, gap: 14 }, shadow.card]}>
            ...
            <View style={{ flexDirection: 'row', gap: 10 }}>
              ...
              <Button label="Save" onPress={() => { const n = parseFloat(hrs); if (!isNaN(n)) { addMomCare({ kind: 'sleep', value: n }); toast('Sleep logged'); } setSleepOpen(false); }} style={{ flex: 1 }} />
            </View>
          </Pressable>
        </Pressable>
      </Modal>
